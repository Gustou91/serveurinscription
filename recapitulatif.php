...
<?php
	...
	$log->trace("Recap - Début Récapitulatif.");
	
	$motifErreur = "";
	$suivant = 1;	// Par défaut, c'est bon.
	...
	function verifNumChq($numChq, $noChq, &$motifErreur, &$suivant) {
		...
		if(	isset($numChq) && !empty($numChq) ) {
			...
			$res = "";
			$motifErreur = "Numéro du ch&egrave;que ".$noChq." non saisi.";
			$suivant = 0;
		}
...
<?php
				...
				$log->trace("Recap - Ajout activité: ".$listActivite);
...
<?php
		...
		if(	isset($_POST["p4_num"]) && !empty($_POST["p4_num"])  &&
			...
			if($p1_num < 10){
				$motifErreur = "Le numéro du chèque 1 doit &ecirc;tre supérieur à 10.";
				$suivant=0;
			}
			if($p2_num < 10){
				$motifErreur = "Le numéro du chèque 2 doit &ecirc;tre supérieur à 10.";
				$suivant=0;
			}
			if($p3_num < 10){
				$motifErreur = "Le numéro du chèque 3 doit &ecirc;tre supérieur à 10.";
				$suivant=0;
			}
			if($p4_num < 10){
				$motifErreur = "Le numéro du chèque 4 doit &ecirc;tre supérieur à 10.";
				$suivant=0;
			}
...
<?php
			...
			$motifErreur = "Une caution est nécessaire.";
			$suivant=0;
...
<?php
		...
		$motifErreur = "Aucune information n'a été renseignée concernant le passeport.";
...
<?php
		...
		$motifErreur = "Aucune information n'a été renseignée concernant la photo.";
		...
		if($certificat=="OUI"){
			if(	isset($_POST["certificat_jj"]) && !empty($_POST["certificat_jj"])  &&
				...
				if (($certificat_jj > 31) or ($certificat_mm > 12)){
					...
					$motifErreur = "Erreur lors de la saisie de la date du certificat médical";
...
<?php
		...
		$motifErreur = "Aucune info sur le certificat médical";
...
<?php
	...
	// Construction de l'url pour reveni sur la page précédente.
	$log->trace("Recap - Recap - Activités: ".$listActivite);
...
<?php
	...
	echo "Merci de sélectionner un membre.<br><input type=\"button\" value=\"Pr&eacute;c&eacute;dent\" onclick=\"Javascript:history.go(-1);\">";
}
?>
</form>
</center>
